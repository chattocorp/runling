...
- Workflow scripts are run with `runling run path/to/workflow.ts "prompt"` and receive one `Runling` containing the framework primitives and invocation details.
