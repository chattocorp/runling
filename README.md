...
token usage, inline input prompts, and the final result in a live TUI on the
terminal's main screen. The completed display remains in normal scrollback.
When either input or output is redirected, Factory automatically uses append-only
logs instead. Use `--log` to force append-only output in an interactive terminal:
