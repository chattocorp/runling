...
import { isWebConfig } from "./web-config.ts";
import { createServer, type RequestListener } from "node:http";

export function parseFactoryWebArguments(argv: readonly string[]) {
...
With no arguments, start the web UI in the current project.`);
  ...
  process.env.FACTORY_WEB_WORKFLOW_CWD = cwd;
  ...
  if (!existsSync(serverUrl))
    throw new Error(
      "Factory web assets are missing. Build the package before running it.",
    );
  ...
  console.log(`Factory listening on http://${host}:${options.port}`);
  const shutdown = () => {
    server.close(() => {
