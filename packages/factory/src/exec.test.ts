...
describe("createExec", () => {
  ...
  test("reports missing executables as failed even in nothrow mode", async () => {
    ...
    await observeFactoryEvents(
      ...
      async () => {
        const result =
          await createExec()`factory-command-that-does-not-exist`.nothrow();
  ...
  test("uses Factory context and allows a per-command cwd override", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "factory-exec-"));
    try {
